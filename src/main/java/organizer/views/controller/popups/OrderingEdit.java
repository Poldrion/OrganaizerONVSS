...
public class OrderingEdit {
    ...
    @FXML
    private RadioButton installationTrue;
    @FXML
    private RadioButton leasingTrue;
    ...
    @FXML
    private TableColumn<Nomenclature, String> id, name, codeKSMId, codeKSMName, technicalRequirementId, technicalRequirementName;
    ...
    private void init(Ordering ordering,
        ...
        if (ordering == null) {
            ...
            this.title.setText("Добавить новую заявку");
        } else {
            this.ordering = ordering;
            this.title.setText("Редактировать текущую заявку");
    ...
    private void save() {
        try {
            ...
            if (validFields()) {
                ...
                close();
                Dialog.DialogBuilder.builder().title("Выполнено успешно.").message("Создание/изменение заявки выполнено успешно.").build().show();
            } else {
                message.setText("Проверьте, все ли обязательные поля (*) заполнены.");
            }
    ...
    private boolean validFields() {
        return !this.nomenclature.getText().isEmpty() && !this.count.getText().isEmpty() && !this.price.getText().isEmpty() && this.date.getValue() != null && !this.unit.getSelectionModel().isEmpty();
    }
