...
public class ExcelUtils {
    ...
    public static void unloadMainTableToExcel(@NotNull ComboBox<String> year, @NotNull ComboBox<String> period, @NotNull Label businessPlanGeneral,
        ...
        try {
            ...
            CellStyle stylePercent = getStyleOnlyNumber(wb, "0.0#%");
            CellStyle styleNumber = getStyleOnlyNumber(wb, "#,##0.00");
            CellStyle centerAlignment = getStyleCenterAlignmentWithBorder(wb);
            CellStyle styleBorderAllAround = getStyleBorderAllAround(wb);
            CellStyle styleBorderAllAroundPercent = getStyleBorderAllAroundAndNumberFormat(wb, "0.0#%");
            CellStyle styleBorderAllAroundNumber = getStyleBorderAllAroundAndNumberFormat(wb, "#,##0.00");
            ...
            saveFile(resultsFolder, "\\Передача потребности на ", year, wb, tempFile);
    ...
    public static void unloadCategoryToExcel(@NotNull CategoryService service, String resultsFolder) {
        try {
            ...
            saveFile(resultsFolder, "\\Категории", wb, tempFile);
        } catch (IOException e) {
            throw new RuntimeException(e);
        } catch (InvalidFormatException e) {
            Dialog.DialogBuilder.builder().title("Неверный формат!!!").message(e.getMessage()).build().show();
        }
    }
    ...
    public static void unloadCodeKSMToExcel(@NotNull CodeKSMService service, String resultsFolder) {
        try {
            ...
            saveFile(resultsFolder, "\\Коды КСМ", wb, tempFile);
        } catch (IOException e) {
            throw new RuntimeException(e);
        } catch (InvalidFormatException e) {
            Dialog.DialogBuilder.builder().title("Неверный формат!!!").message(e.getMessage()).build().show();
    ...
    public static void unloadSubcategoryToExcel(@NotNull SubcategoryService service, String resultsFolder) {
        try {
            ...
            saveFile(resultsFolder, "\\Подкатегории", wb, tempFile);
        } catch (IOException e) {
            throw new RuntimeException(e);
        } catch (InvalidFormatException e) {
            Dialog.DialogBuilder.builder().title("Неверный формат!!!").message(e.getMessage()).build().show();
    ...
    public static void unloadTRToExcel(@NotNull TechnicalRequirementService service, String resultsFolder) {
        try {
            ...
            CellStyle styleBorderAllAround = getStyleBorderAllAround(wb);
            CellStyle styleBorderAllAroundDate = getStyleBorderAllAroundAndDateFormat(wb, "dd.MM.yyyy");
            ...
            saveFile(resultsFolder, "\\ОЛ_ТТ", wb, tempFile);
        } catch (IOException e) {
            throw new RuntimeException(e);
        } catch (InvalidFormatException e) {
            Dialog.DialogBuilder.builder().title("Неверный формат!!!").message(e.getMessage()).build().show();
    ...
    public static void unloadNomenclatureToExcel(@NotNull NomenclatureService service, String resultsFolder) {
        try {
            ...
            saveFile(resultsFolder, "\\Номенклатура", wb, tempFile);
        } catch (IOException e) {
            throw new RuntimeException(e);
        } catch (InvalidFormatException e) {
            Dialog.DialogBuilder.builder().title("Неверный формат!!!").message(e.getMessage()).build().show();
    ...
    public static void unloadBPToExcel(@NotNull BusinessPlanService service, String resultsFolder) {
        try {
            ...
            saveFile(resultsFolder, "\\Бизнес-план", wb, tempFile);
        } catch (IOException e) {
            throw new RuntimeException(e);
        } catch (InvalidFormatException e) {
            Dialog.DialogBuilder.builder().title("Неверный формат!!!").message(e.getMessage()).build().show();
    ...
    public static void unloadOrderingToExcel(@NotNull ComboBox<String> year, @NotNull ComboBox<String> period, @NotNull ComboBox<String> category, @NotNull RadioButton allEquipment, RadioButton onlyLeasing, RadioButton withoutLeasing, CheckBox emptyCategory, CheckBox onlyCreatedOrdering, CheckBox onlyUncreatedOrdering, TableView<Ordering> table, String resultsFolder) {
        try {
            ...
            if (allEquipment.isSelected()) {
                ...
                row.createCell(2).setCellValue("Неопределенная группа оборудования");
            }

            // Отображение пустых заявок
            row = sheet.getRow(4);
            if (emptyCategory.isSelected()) {
                row.createCell(2).setCellValue("Да");
            } else {
                row.createCell(2).setCellValue("Нет");
            }

            // Отображение только фактически переданных заявок (с номерами заявок)
            row = sheet.getRow(5);
            if (onlyCreatedOrdering.isSelected()) {
                row.createCell(2).setCellValue("Только созданные заявки");
            } else if (onlyUncreatedOrdering.isSelected()) {
                row.createCell(2).setCellValue("Только не созданные заявки");
            } else {
                row.createCell(2).setCellValue("Все заявки");
            }
            ...
            saveFile(resultsFolder, "\\Заявки ОНВСС на ", year, wb, tempFile);
        } catch (IOException | InvalidFormatException e) {
            throw new RuntimeException(e);
        }
        ...
        try {
            ...
            for (int rowNum = rowStart; rowNum <= rowEnd; rowNum++) {
                ...
                try {
                    ...
                    ordering.setRequiresInstallation(getCellText(row.getCell(13)).equals("Да"));
                    ordering.setLeasing(getCellText(row.getCell(14)).equals("Да"));
            ...
            if (ERROR_SET.size() > 0) {
                Path fileErrorPath = Path.of("errorLog\\Ошибки загрузки от " + LocalDate.now().format(dtf) + ".log");
                File fileError;
                ...
                Dialog.DialogBuilder.builder().title("Ошибка при загрузке файла!").message("При загрузке файла было выявлено ошибок: " + ERROR_COUNT + ".").build().show();
    ...
    public static void unloadToOrderingTemplate(ComboBox<String> year, @NotNull SubcategoryService subcategoryService, @NotNull TableView<Ordering> table, String resultsFolder) {
            ...
            for (Subcategory subcategory : subcategoryList) {
                ...
                if (tempList.size() > 0) {
                    ...
                    for (int i = 0; i < tempList.size(); i++) {
                        ...
                        // Завод
                        cell = row.createCell(5);
                        cell.setCellValue("N01F");
                        // Склад
                        cell = row.createCell(6);
                        cell.setCellValue("VIRT");
                        // Номер материала
                        cell = row.createCell(7);
                        ...
                        // Группа закупок
                        cell = row.createCell(12);
                        cell.setCellValue("SN1");
                        // Закупочная организация
                        cell = row.createCell(13);
                        cell.setCellValue("1322");
                        // Завод-постащик
                        cell = row.createCell(14);
                        cell.setCellValue("N01F");
                        // Отпускающий склад
                        cell = row.createCell(15);
                        cell.setCellValue("M000");
                        // Дата поставки
                        cell = row.createCell(16);
                        cell.setCellValue(tempList.get(i).getDate().format(dtf));
                        // ПФМ
                        cell = row.createCell(29);
                        cell.setCellValue("CFR1322010124000");
                        // ФП
                        cell = row.createCell(30);
                        cell.setCellValue("2.9.001");
                        // Вид деятельности
                        cell = row.createCell(43);
                        cell.setCellValue("03");
                        // Подразделение-заказчик
                        cell = row.createCell(48);
                        cell.setCellValue("10048791");
                        // Дата вовлечения
                        ...
                        cell.setCellValue("194238");

                        if (tempList.get(i).isRequiresInstallation()) {
                            // ШМР
                            setRequiresInstallationRow(sheet, position, i, 1, "3023301", "Шеф-монтажные работы НПО", tempList);

                            // ПНР
                            setRequiresInstallationRow(sheet, position, i, 2, "3021595", "НаладочныеПусконаладочныеРаботы на НПО", tempList);
                        }
                    }
                    // Лист "Обоснование плановой стоимости"
                    sheet = wb.getSheet("Обоснование плановой стоимости");
                    for (int i = 0; i < tempList.size(); i++) {
                        int position = 10;
                    ...
                    saveFileTemplates(resultFolderForTemplates, "\\Шаблон загрузки на ", year, subcategory, wb, tempFile);
                    tempList.clear();
                    wb.close();
                    pkg.close();
                }
        ...
        Dialog.DialogBuilder.builder().title("Выполнено успешно.").message("Формирование шаблонов загрузки выполнено успешно.").build().show();
    }
     ...
     */
    private static void saveFile(String resultsFolder, String x, @NotNull ComboBox<String> year, @NotNull XSSFWorkbook wb, File tempFile) throws IOException {
        String resultFile = resultsFolder + x + year.getSelectionModel().getSelectedItem() + " год от " + LocalDate.now().format(dtf) + ".xlsx";
     ...
     * @param tempFile      временная копия файла шаблона (нужна для предотвращения порчи исходного шаблона)
     */
    private static void saveFileTemplates(String resultsFolder, String x, @NotNull ComboBox<String> year, @NotNull Subcategory subcategory, @NotNull XSSFWorkbook wb, File tempFile) throws IOException {
        String resultFile = resultsFolder + x + year.getSelectionModel().getSelectedItem() + " для " + subcategory.getName() + " " + LocalDate.now().format(dtf) + ".xlsx";
     ...
     */
    private static @NotNull File getTempFile(String tempFileTemplate, String fileTemplate) throws IOException {
    ...
    private static void notFoundNomenclature(int rowNum, @NotNull Row row) {
        String temp = String.format("""
                Строка excel №%s не загружена!
                Не найдена сл. номенклатура:
                номер заявки - %s, позиция - %s, код КСМ - %s, наименование МТР - %s, сегмент запаса - %s, ЕИ - %s, кол-во - %s, цена(руб. без НДС) - %s, дата поставки - %s, требуется ШМР/ПНР - %s, лизинг - %s, примечение - %s.
    ...
    private static @NotNull CellStyle getStyleBorderAllAroundAndNumberFormat(@NotNull XSSFWorkbook wb, String format) {
    ...
    private static @NotNull CellStyle getStyleBorderAllAround(@NotNull XSSFWorkbook wb) {
    ...
    private static @NotNull CellStyle getStyleCenterAlignmentWithBorder(@NotNull XSSFWorkbook wb) {
    ...
    private static @NotNull CellStyle getStyleOnlyNumber(@NotNull XSSFWorkbook wb, String format) {
        CellStyle stylePercent = wb.createCellStyle();
        stylePercent.setDataFormat(wb.createDataFormat().getFormat(format));
        return stylePercent;
    }

    private static @NotNull CellStyle getStyleBorderAllAroundAndDateFormat(@NotNull XSSFWorkbook wb, String format) {
    ...
    public static FileChooser getFileChooser(String fileName) {
        ...
        fileChooser.setTitle("Выберите файл " + fileName + " для загрузки в БД");
